...
(ref:Table1Caption) List of head dimensions, measurement descriptions and specification of caliper jaw placement.
\ \
\ \
...
\ \
\ \
(ref:Table2Caption) Primers used in the development of a genetic test for determining sablefish sex.
\ \
\ \
...
```{r table3, results="asis",echo=FALSE, warning=FALSE, message=FALSE}
    ...
    sample <- paste("select  CASE WHEN TRIP_ID = 0 THEN 'Salmon' WHEN trip_id = 80378 THEN '2016 WCHG'  ",
                    ...
                    " SUM(ID) AS ID, SUM(SL) AS SL, SUM(PP) AS PP, SUM(PH) AS PH, ",
   ...
   last.line  <-   c('Total', sum(samplecnt$FL), sum(samplecnt$RW),
                         ...
                         sum(samplecnt$PP), sum(samplecnt$PH),
...
(ref:Table4Caption) Table of sample size, mean and standard deviation for predictor and response variables for each head dimension.
\ \
...
```{r table4, echo=FALSE}
    ...
    head.data    <-  data.frame(measurement="Upper jaw length (UJ)",n,mn, std, fl.nme, mnfl, stdfl)
    ...
    head.data[2,] <- c("Eye diameter (ED)",n,mn, std, fl.nme, mnfl, stdfl)
    ...
    head.data[3,] <- c("InterOrbital distance (ID)",n, mn, std, fl.nme, mnfl, stdfl)
    ...
    head.data[4,] <-c("Snout length (SL)",n, mn, std, fl.nme, mnfl, stdfl)
    ...
    head.data[5,] <- c("Post orbital to preoperculum length (PP)",n,mn, std, fl.nme, mnfl, stdfl)
...
```
(ref:Table5Caption) Ease of use and repeat ability considerations for each measurement.number, ease of use and repeatability five point ranking where  5-Great; 4- Good; 3-Moderate; 2-Questionable; 1-Terrible. 
...
(ref:Table6Caption) Estimated slope and 
